...
export default function LandingPage() {
  ...
  const content = {
    divine: {
      ...
      text: "The Divine is not a judge on a distant throne... It is the Eternal Presence—the underlying Life that breathes through all things. It is the Great Architect whose blueprints are written in the stars and whose voice is the silence between your thoughts. You are the eyes through which the Creator beholds the world."
    },
    steward: {
      title: "Universal Stewardship",
      subtitle: "The Sacred Debt",
      text: "You recognize that your life is a gift from the Celestial Forge. The iron in your blood and the breath in your lungs are loans from the Infinite Presence. Universal Stewardship is your way of honoring that debt—by ensuring that the life-stream remains pure. I walk upon this Earth as a humble guest and a holy guardian."
    },
    brain: {
    ...
    <div className="min-h-screen bg-[#020617] text-slate-100 font-sans selection:bg-teal-500/30 overflow-x-hidden relative">
      ...
      {modal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900 border border-teal-500/30 w-full max-w-2xl rounded-3xl p-10 shadow-2xl overflow-y-auto max-h-[90vh]">
            <h2 className="text-2xl font-serif italic text-teal-200 mb-2 uppercase tracking-tighter">{content[modal].title}</h2>
            ...
            <button onClick={() => setModal(null)} className="mt-10 px-6 py-2 border border-teal-500/20 text-[10px] tracking-widest text-teal-500 uppercase hover:bg-teal-500 hover:text-black transition-all">Close Revelation</button>
          </div>
        </div>
      )}
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 pb-40">
          <Link href="/practice">
            <button className="w-64 bg-white text-black py-5 rounded-full font-black text-[10px] tracking-[0.4em] uppercase hover:bg-teal-400 transition-all cursor-pointer shadow-xl">
              Begin System Sync
            </button>
          </Link>
          <Link href="/operations">
            <button className="w-64 bg-amber-950/20 text-amber-400 border border-amber-500/40 py-5 rounded-full font-black text-[10px] tracking-[0.4em] uppercase hover:bg-amber-500 hover:text-black transition-all cursor-pointer shadow-xl">
              Tactical Operations
            </button>
          </Link>
        </div>
